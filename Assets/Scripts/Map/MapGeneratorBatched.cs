...
namespace RollABall.Map
{
    ...
    public class MapGeneratorBatched : MonoBehaviour
    {
        ...
        [SerializeField, HideInInspector] private GameObject roadPrefab, buildingPrefab, areaPrefab, collectiblePrefab, goalZonePrefab, playerPrefab;

        [Header("Road Materials by Type")]
        [SerializeField] private Material roadMotorway, roadPrimary, roadSecondary, roadResidential, roadFootway, roadDefault;

        [Header("Building Materials by Type")]
        [SerializeField] private Material residentialMaterial, industrialMaterial, commercialMaterial, officeMaterial, defaultBuildingMaterial;

        [Header("Area Materials")]
        [SerializeField] private Material parkMaterial, waterMaterial, forestMaterial, grassMaterial, defaultAreaMaterial;

        [Header("Steampunk Decoration Prefabs")]
        [SerializeField, HideInInspector] private GameObject gearPrefab, steamPipePrefab, chimneySmokeParticles;

        [Header("Batching Settings")]
        [SerializeField] private bool enableBatching = true, enableStaticBatching = true, separateColliders = true, batchAreas = true;

        [Header("Road Settings")]
        [SerializeField] private float roadHeightOffset = 0.05f;
        [SerializeField] private bool enableRoadColliders = false, enableFootwayColliders = false;
        ...
        [SerializeField] private int maxBuildingsPerFrame = 5, maxRoadSegmentsPerFrame = 10;

        [Header("Debug")]
        [SerializeField] private bool debugMode = true;

        private Transform mapContainer, batchedRoadsContainer, batchedBuildingsContainer, batchedAreasContainer, collidersContainer;
        ...
        private List<ColliderData> roadColliders = new(), buildingColliders = new();
        ...
        private IEnumerator CollectRoadMeshes()
        {
            ...
            foreach (var road in currentMapData.roads)
            {
                ...
                {
                    var mesh = CreateRoadSegmentMesh(road.nodes[i], road.nodes[i + 1], highwayType);
                    if (mesh.HasValue)
                    {
                        roadMeshesByMaterial[GetRoadMaterialKey(highwayType)].Add(mesh.Value);
                        if ((enableRoadColliders && highwayType != "footway") || (enableFootwayColliders && highwayType == "footway"))
                            roadColliders.Add(CreateColliderData(mesh.Value, "Road"));
                    }
        ...
        private CombineInstance? CreateRoadSegmentMesh(OSMNode start, OSMNode end, string highwayType)
        {
            ...
            startPos.y = endPos.y = roadHeightOffset;
            return MeshUtilities.CreateRoadSegmentMesh(startPos, endPos, GetRoadWidth(highwayType), 0.1f);
        }
        ...
        private IEnumerator CollectBuildingMeshes()
        {
            ...
            foreach (var b in currentMapData.buildings)
            {
                ...
                if (mesh.HasValue)
                {
                    buildingMeshesByMaterial[GetBuildingMaterialKey(b.buildingType)].Add(mesh.Value);
                    buildingColliders.Add(CreateColliderData(mesh.Value, "Building"));
                }
                if (++processed >= maxBuildingsPerFrame) { processed = 0; yield return null; }
            }
        }
        ...
        private CombineInstance? CreateBuildingMesh(OSMBuilding b)
        {
            ...
            return MeshUtilities.CreateBuildingMesh(CalculateBuildingCenter(b), size, CalculateBuildingRotation(b));
        }
        ...
        private IEnumerator CollectAreaMeshes()
        {
            ...
            int processed = 0;
            foreach (var a in currentMapData.areas)
            {
                if (a.nodes.Count < 3) continue;
                a.DetermineAreaType();
                var mesh = CreateAreaMesh(a);
                if (mesh.HasValue)
                    areaMeshesByMaterial[GetAreaMaterialKey(a.areaType)].Add(mesh.Value);
        ...
        private IEnumerator BatchMeshesByMaterial<TKey>(Dictionary<TKey, List<CombineInstance>> col, Transform container, System.Func<TKey, Material> getMat, string prefix)
        {
        ...
        private IEnumerator CreateSeparateColliders()
        {
            foreach (var data in roadColliders.Concat(buildingColliders))
            {
                ...
                if (data.mesh != null) obj.AddComponent<MeshCollider>().sharedMesh = data.mesh; else obj.AddComponent<BoxCollider>();
        ...
        private IEnumerator GenerateGroundPlane()
        {
            ...
            float size = Mathf.Max((float)(currentMapData.bounds.GetWidth() * currentMapData.scaleMultiplier), (float)(currentMapData.bounds.GetHeight() * currentMapData.scaleMultiplier));
        ...
        private IEnumerator GenerateAreas()
        {
            ...
            foreach (var a in currentMapData.areas)
            {
                ...
                cube.GetComponent<MeshRenderer>().material = GetAreaMaterial(GetAreaMaterialKey(a.areaType));
        ...
        private void SetPlayerSpawnPosition()
        {
            ...
            GameObject player = GameObject.FindGameObjectWithTag("Player") ?? Instantiate(playerPrefab, playerSpawnPosition, Quaternion.identity);
        ...
        private float GetRoadWidth(string type) => type switch { "motorway" => 5.5f, "primary" => 4f, "secondary" => 3f, "residential" => 2f, "footway" => 1f, _ => 2f };
        private RoadCategory GetRoadMaterialKey(string type) => type switch { "motorway" => RoadCategory.Motorway, "primary" => RoadCategory.Primary, "secondary" => RoadCategory.Secondary, "residential" => RoadCategory.Residential, "footway" => RoadCategory.Footway, _ => RoadCategory.Default };
        private BuildingCategory GetBuildingMaterialKey(string type) => type switch { "industrial" => BuildingCategory.Industrial, "commercial" => BuildingCategory.Commercial, "office" => BuildingCategory.Office, "residential" => BuildingCategory.Residential, _ => BuildingCategory.Default };
        private AreaCategory GetAreaMaterialKey(string type) => type switch { "park" => AreaCategory.Park, "water" => AreaCategory.Water, "forest" => AreaCategory.Forest, "grass" => AreaCategory.Grass, _ => AreaCategory.Default };
        private Material GetRoadMaterial(RoadCategory key) => key switch { RoadCategory.Motorway => roadMotorway ?? roadDefault, RoadCategory.Primary => roadPrimary ?? roadDefault, RoadCategory.Secondary => roadSecondary ?? roadDefault, RoadCategory.Residential => roadResidential ?? roadDefault, RoadCategory.Footway => roadFootway ?? roadDefault, _ => roadDefault };
        private Material GetBuildingMaterial(BuildingCategory key) => key switch { BuildingCategory.Residential => residentialMaterial ?? defaultBuildingMaterial, BuildingCategory.Industrial => industrialMaterial ?? defaultBuildingMaterial, BuildingCategory.Commercial => commercialMaterial ?? defaultBuildingMaterial, BuildingCategory.Office => officeMaterial ?? defaultBuildingMaterial, _ => defaultBuildingMaterial };
        private Material GetAreaMaterial(AreaCategory key) => key switch { AreaCategory.Park => parkMaterial ?? defaultAreaMaterial, AreaCategory.Water => waterMaterial ?? defaultAreaMaterial, AreaCategory.Forest => forestMaterial ?? defaultAreaMaterial, AreaCategory.Grass => grassMaterial ?? defaultAreaMaterial, _ => defaultAreaMaterial };
        ...
        private Vector3 CalculateBuildingSize(OSMBuilding b) { var pts = b.nodes.Select(n => currentMapData.LatLonToWorldPosition(n.lat, n.lon)).ToList(); Vector3 min = pts.Aggregate(Vector3.positiveInfinity, Vector3.Min); Vector3 max = pts.Aggregate(Vector3.negativeInfinity, Vector3.Max); return max - min; }
        private Quaternion CalculateBuildingRotation(OSMBuilding b) { if (b.nodes.Count < 2) return Quaternion.identity; var p1 = currentMapData.LatLonToWorldPosition(b.nodes[0].lat, b.nodes[0].lon); var p2 = currentMapData.LatLonToWorldPosition(b.nodes[1].lat, b.nodes[1].lon); var dir = (p2 - p1).normalized; return dir.magnitude > 0.1f ? Quaternion.Euler(0, Mathf.Atan2(dir.x, dir.z) * Mathf.Rad2Deg, 0) : Quaternion.identity; }
        private Vector3 CalculateAreaCenter(OSMArea a) { int count = a.IsClosed() ? a.nodes.Count - 1 : a.nodes.Count; return a.nodes.Take(count).Select(n => currentMapData.LatLonToWorldPosition(n.lat, n.lon)).Aggregate(Vector3.zero, (sum, v) => sum + v) / count; }
        private Vector3 CalculateAreaSize(OSMArea a) { int count = a.IsClosed() ? a.nodes.Count - 1 : a.nodes.Count; var pts = a.nodes.Take(count).Select(n => currentMapData.LatLonToWorldPosition(n.lat, n.lon)).ToList(); Vector3 min = pts.Aggregate(Vector3.positiveInfinity, Vector3.Min); Vector3 max = pts.Aggregate(Vector3.negativeInfinity, Vector3.Max); Vector3 size = max - min; size.x = Mathf.Max(size.x, 2f); size.z = Mathf.Max(size.z, 2f); return size; }
        private ColliderData CreateColliderData(CombineInstance ci, string name) => new ColliderData { name = name, mesh = ci.mesh, position = ci.transform.GetColumn(3), rotation = ci.transform.rotation, scale = ci.transform.lossyScale };
        private void LogPerformanceStats() => Log($"Road meshes: {roadMeshesByMaterial.Sum(k => k.Value.Count)} | Building meshes: {buildingMeshesByMaterial.Sum(k => k.Value.Count)}");
        private void FailGeneration(string msg) { Debug.LogError(msg); OnGenerationError?.Invoke(msg); isGenerating = false; }
        private void Log(string msg) { if (debugMode) Debug.Log("[MapGen] " + msg); }

        [System.Serializable] public struct ColliderData { public string name; public Mesh mesh; public Vector3 position; public Quaternion rotation; public Vector3 scale; }
    }
}
